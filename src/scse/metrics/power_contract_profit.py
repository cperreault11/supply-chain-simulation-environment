...
class ProfitCalculations():
    ...
    def compute_reward(self, state, action):
        if action['type'] == 'advance_time':
            ...
            print (','.join([str(total_cost), str(state['true_demand']),str(state['demand_sd']), str(sum(bid['sd'] for bid in state['bids'])), str(sum(bid['quantity'] for bid in state['bids'] if bid['bidder'] == 'renewable'))]))
